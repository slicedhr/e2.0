...
(function () {
   ...
   *
   */
  angular.module('clientes').directive('clientesForm', clientesForm);
  ...
  function clientesForm() {
    ...
    return {
      ...
      controller: function controller($scope, $rootScope, $mdToast, $filter, AppService) {
        ...
        if (Object.keys(self.data).length > 0) for (var i = 0; i < self.validations.length; i++) {
        ...
        this.deleteCategory = function (item) {
          var isProductCategory = arguments.length <= 1 || arguments[1] === undefined ? false : arguments[1];

          var array = isProductCategory ? self.categoriasproductosArr : self.data.categoriascliente;
        ...
        this.validateSelectionCategoria = function (item) {
          ...
          if (item) {
            ...
            if ($filter('getByID')(self.categoriasproductosArr, item.id)) return;

            self.categoriasproductosArr.push(item);
          }
        };

        this.selectCategoriaCliente = function () {
        ...
        this.addCategoriaToCliente = function () {
          ...
          if (self.categoriaProductoActived) category.categoriasproductos = self.categoriasproductosArr;

          if (!$filter('getByID')(self.data.categoriascliente, self.selectedcategoriacliente)) self.data.categoriascliente.push(category);
        ...
        this.validateNit = function (ev) {

          if (self.data.nit) AppService.http(AppService.setConfig({
            ...
            if (data.results.length > 0) {
              ...
              $mdToast.show($mdToast.simple().textContent('Ya existe un cliente con esa identificación:  ' + data.results[0].razon_social).position('top left').hideDelay(5000));
            } else self.formValidate = true;
          });else self.formValidate = false;
        };
...
//# sourceMappingURL=clientes-form-directive.js.map
